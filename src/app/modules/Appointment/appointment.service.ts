  ...
  const result = await prisma.appointment.findMany({
    ...
    orderBy:
      options.sortBy && options.sortOrder
        ? { [options.sortBy]: options.sortOrder }
        : { createdAt: "desc" },
    include: user?.role === UserRole.PATIENT ? {doctor : true, schedule : true} : {patient: {include :{medicalReport :true,patientHealthData:true}} , schedule: true}
  });
...
export const AppointmentService = {
  ...
  getMyAppointmentFromDB,getAllAppointmentFromDB
};
